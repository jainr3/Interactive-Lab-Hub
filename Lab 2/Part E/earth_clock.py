...
def crop_image(scale):
    print(f"Cropping image at scale {scale}.")
    im = Image.open('earth.png')
    width, height = im.size
    left = scale*300#10#80
    top = scale*150#125#150
    right = scale*700#305#230
    bottom = scale*350#300#250
...
while True:
    # Every 30 minutes update the fetched image
    if (time.time() - last_fetched) > 30 * 60:
        update_image(driver)
        crop_image(scale)
        update = True
        last_fetched = time.time()
